...
describe("API Dispatcher", function() {
  ...
  describe ("Queue with timeouts", function() {
    ...
    before(function(done) {
      ...
      server.respondWith("GET", baseURL + "beacon-list", [200, munros]);
      server.respondWith("GET", baseURL + "forecasts", [200, forecasts]);
      server.respondWith("POST", baseURL + "users", [201, user]);

      new ApiRequest().makeGetRequest(baseURL + "munros", false, callback);
      new ApiRequest().makePostRequest(baseURL + "users", user_params, true, function() {
        status = arguments[0];
        response = arguments[1];
        done();
      });
      new ApiRequest().makeGetRequest(baseURL + "forecasts", false, callback);
    });
  ...
  describe ("Queue with timeout and pause", function() {
    ...
    before(function(done) {
      ...
      new ApiRequest().makeGetRequest(baseURL + "munros", false, callback);
      new ApiRequest().makePostRequest(baseURL + "users", user_params, true, callback);
      new ApiRequest().makeGetRequest(baseURL + "forecasts", false, callback);
      setTimeout(function(){
        dispatcher._onPause();
        setTimeout(function(){
